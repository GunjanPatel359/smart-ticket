...
class AssignmentService:
    ...
    def process_ticket_assignment(self, request_data: Dict[str, Any]) -> TicketAssignmentResponse:
        ...
        try:    
            ...
            extracted_skill_names = self._extract_skills_from_ticket(ticket, available_skill_names)
            ...
            return TicketAssignmentResponse(
                success=True,
                selected_technician_id=selected_technician.id if selected_technician else None,
    ...
    def _extract_and_validate_ticket(self, request_data: Dict[str, Any]) -> Ticket:
        ...
        try:
            ...
            required_fields = ["subject", "description", "requesterId", "id"]
            ...
            ticket = Ticket(
                ...
                requester_id=ticket_data["requesterId"],
                ...
                resolution_date=ticket_data.get("resolution_date"),
    ...
    def _get_available_skills(self) -> List[Skill]:
        ...
        """
        try:
            logger.info(f"Fetching available skills from: {os.environ.get('BACKEND_SERVER_URL','http://localhost:5001')}/api/v1/skills/all")
            
            response = requests.get(f"{os.environ.get('BACKEND_SERVER_URL','http://localhost:5001')}/api/v1/skills/all", timeout=10)
    ...
    def _get_technicians(self, extracted_skills: List[Skill], by_skills: bool = True) -> List[Technician]:
        ...
        try:
            ...
            if by_skills:
                ...
                technicians_url = f"{os.environ.get('BACKEND_SERVER_URL','http://localhost:5001')}/api/v1/technicians/by-skills"
                ...
                # Prepare query parameters
                technicians_url = f"{os.environ.get('BACKEND_SERVER_URL','http://localhost:5001')}/api/v1/technicians/all"
                        ...
                        for skill_data in tech_data['skills']:
                            if isinstance(skill_data, dict) and 'id' in skill_data:
                                skill_obj = SkillObject(
                                    id=int(skill_data['id']) if 'id' in skill_data and skill_data['id'] is not None else 0,
    ...
    def _notify_extracted_skills(self, extracted_skill_names: Dict[str,Any], existing_extracted_skills: List[Skill]):
        ...
        try:
            ...
            if extracted_skill_names.get("new_skills"):
                for skill in extracted_skill_names.get("new_skills") or []:
                    data.append({
                        "name": skill["name"],
                        "description": skill["description"],
                    })
            ...
            response = requests.post(f"{os.environ.get('BACKEND_SERVER_URL','http://localhost:5001')}/api/v1/tickets/process-skills", json={"ticket_id": self._ticket_id, "skills": data}, timeout=10)
            response.raise_for_status()
    ...
    def get_assignment_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "required_request_fields": ["subject", "description", "requesterId"],
    ...
    def validate_request_data(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            required_ticket_fields = ["subject", "description", "requesterId"]
            ...
            # Check requester_id
            if "requesterId" in ticket_data:
                try:
                    requester_id = int(ticket_data["requesterId"])
                    if requester_id <= 0:
                        validation_result["valid"] = False
                        validation_result["errors"].append("requesterId must be a positive integer")
                except (ValueError, TypeError):
                    validation_result["valid"] = False
                    validation_result["errors"].append("requesterId must be a valid integer")
